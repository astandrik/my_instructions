...
- Repeated failure across attempts → Reflexion: write a short "lesson learned" (1–3 bullets: what failed, why, what to do next time); if an explicitly enabled durable notes facility is available, store it as a `Lesson:` entry and never include secrets, tokens, or PII.
